...
use App\Http\Controllers\MoneyPointJournalEntryController;
...
class CreateJournalEntryJob implements ShouldQueue
{
    ...
    protected $mPJournalEntryController;
    ...
    {
        $mPJournalEntryController = new MoneyPointJournalEntryController();
        $netsuite_connector = new NetsuiteConnectorController();

        $this->requestData = $requestData;
        $this->mPJournalEntryController =  $mPJournalEntryController;
    ...
    public function handle()
    {
        ...
        try{
            ...
            foreach ($data['journal'] as $data_return){
                $redemption_number = isset($data_return['reference_number']) ? $data_return['reference_number'] : "Missing Reference Number";
                $lms_no = isset($data_return['lms_no']) ? $data_return['lms_no'] : "  ";

                try{
                    $redemption = $this->mPJournalEntryController->findJournalEntry($company_id,$environment,$redemption_number,$lms_no);
                    // dd($redemption);
                    if(!$redemption['message']){

                        $failed_redemptions [] = ['reference_number'=>$redemption_number,'message'=>'Check Reference provided on the Payload'];
                    }elseif($redemption['statusCode'] == 202) {
                        $existing_redemptions [] =  $redemption_number;

                    } else {
                        $url = "https://" . $account_number . ".restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=customscript_jn_create_je&deploy=customdeploy_jn_create_je";
                        $method = "POST";
                        $data = "";
                        ...
                        if ($send_request['statusCode'] != 200) {
                            ...
                            $failed_redemptions [] = ['reference_number'=>$redemption_number,'message'=>$message];
                    ...
                    $failed_redemptions [] = ['reference_number'=>$redemption_number,'message'=> $ex->getMessage() .' Line'.$ex->getLine()];
                }
            }
        ...
        $handler = fopen("journal_entry_creation_response_sent" . date('d-m-Y') . ".txt", "a");
